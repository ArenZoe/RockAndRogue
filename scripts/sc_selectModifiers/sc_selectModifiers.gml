...
function selectModifiers(roundNumber,setlistIndex){
	...
	var modifierCount = modifierTable[clamp((roundNumber-1),0,array_length(modifierTable)),setlistIndex];
	
	var availableModifiersXCommon = [
		Modifiers.speedIncrease110,
		Modifiers.highwayLength100,
	...
	for (var i = 0; i < modifierCount; i++){
		...
		if (assignedModifier = Modifiers.highwayLength100 
		or assignedModifier = Modifiers.highwayLength80 
		or assignedModifier = Modifiers.highwayLength60
		){
			if (array_get_index(availableModifiersXCommon, Modifiers.highwayLength100) != -1){
				array_delete(availableModifiersXCommon, array_get_index(availableModifiersXCommon, Modifiers.highwayLength100),1);
			}
